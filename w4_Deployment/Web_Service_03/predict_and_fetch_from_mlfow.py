...
# RESTful API endpoints in Flask apps are URL routes (e.g., '/predict', '/items/<id>') that handle HTTP requests (GET, POST, PUT, DELETE).
# Each endpoint corresponds to a function (view) that processes the request and returns a response, typically in JSON format.
# Example:
# @app.route('/predict', methods=['POST'])
# def predict_endpoint():
#     # handle POST request and return prediction result
